...
// Noop is a no-op notifier that logs operations but takes no action.
...
func (n *Noop) Cordon(ctx context.Context, nodeID string, reason string) error {
	n.logger.Info("node cordoned (no notifier configured)",
...
func (n *Noop) Uncordon(ctx context.Context, nodeID string) error {
	n.logger.Info("node uncordoned (no notifier configured)",
		slog.String("node_id", nodeID),
	)
	return nil
}
...
func (n *Noop) Drain(ctx context.Context, nodeID string, reason string) error {
	n.logger.Info("node drain requested (no notifier configured)",
		slog.String("node_id", nodeID),
		slog.String("reason", reason),
	)
	return nil
...
// Ensure Noop implements Notifier.
